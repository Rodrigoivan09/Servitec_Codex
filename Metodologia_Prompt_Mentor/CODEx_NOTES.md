...
- 2025-10-29 — **Pipeline MariaDB en rodev**: el workflow de despliegue declara ahora la red `servitec_network`, el volumen persistente `servitec_db_data` y un contenedor `mariadb:10.11` antes de arrancar la app (`.github/workflows/deploy-servitec.yml:13-176`). Se agregaron validaciones para que existan los secretos `SERVITEC_DB_ROOT_PASSWORD`, `SERVITEC_DB_NAME`, `SERVITEC_DB_USER` y `SERVITEC_DB_PASSWORD`, se espera a que `mysqladmin ping` confirme disponibilidad y luego se ejecuta `servitec_app` con `SPRING_DATASOURCE_URL` apuntando a `servitec_db`. Replicar este patrón cuando el despliegue requiera servicios auxiliares en la misma VM.
